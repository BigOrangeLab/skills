...
- **Key authorisation**: The SSH Gateway key's _public_ half must be added to the WP Engine portal under **Users → Your Profile → SSH Keys** — adding it only to `authorized_keys` on the server is not sufficient.
